...
def draw_contours(result, img, cntrs, image_name):
    ...
    for c in cntrs:
        ...
        #if w/width > 0.0302 and h/height > 0.0213:
        if True:
            if is_gibberish(text):
                # Likely to be an image
                new_image = img[y:y+h, x:x+w]
                cv2.imwrite("TempImages/" + str(count) + ".jpg" , new_image)
                document_data_list.append(("TempImages/" + str(count) + ".jpg", "image", y, pseudo_text))
                count = count + 1
            else:
                # Likely to be a text
...
def generate_document(filename, documentdir):
    ...
    # Remove /images from image_name
    image_name = image_name.split('/', 1)[1]
    # Test paper name found in /images
...
=======
>>>>>>> a39552ecda8734e2cfc8726e6a6bcda9ed873c3e
    pg_cntr = pg_cntr + 1
