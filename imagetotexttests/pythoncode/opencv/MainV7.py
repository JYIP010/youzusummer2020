# V7: Tried improving page segmentation, by introducing preprocessing(find_qn_coords)
# 1. Not very accurate in detecting questions
# 2. Uses the (4) answer option in mcq to detect question segmentation
# 3. Uses (a), (b), (c), (d), in structured questions to detect question segmenetation
import cv2
import numpy as np
import pytesseract
...
def draw_contours(result, img, cntrs, image_name, finding_qn):
    ...
    for c in cntrs:
        ...
        if w / width > 0.1 and h / height > 0.001 and y / height < 0.95 and w / h < 25:  # and (x/width < 0.4 or x/width > 0.5)
            ...
            if linesp is not None or is_gibberish(text, finding_qn):
...
def write_data_to_document(document_data_list, document, filename, qn_coord):
    ...
    for i in range(len(document_data_list)):
        ...
        # STEP 2: Add qn number if current contour exceeds the page number or y_coord of the current question
        # (pg_num, y_coord, qn_num, qn_section)
        if qn_num < len(qn_coord):
            current_qn_tuple = qn_coord[qn_num]
            if pg_num > current_qn_tuple[0] or (pg_num == current_qn_tuple[0] and y_coord > current_qn_tuple[1]):
                qn_num = qn_num + 1
...
def generate_document(filename, documentdir, qn_coord):
    ...
    y_tolerance = m.floor(0.009 * height) # previously 0.014964
    ...
    # type is "text" or "image"
    # y_coord contains y coordinates of the text or image
    document_data_list = draw_contours(result, img, cntrs, image_name, True)
...
def find_qn_coords(filenames_list):
    global total_pages
    qn_coord = []
    qn_coord.append((0, 0, 0 , 0))
    pg_num = 0
    qn_num = 1
    last_section_found_number = -1
    last_y_coord = -1
    last_pg_num = 0
    ...
    for filename in filenames_list:
        ...
        for i in range(len(document_data_list)):
            ...
            pseudo_text = data[3]
            if True:
                # For Structured questions, check for sections
                possible_sections = [r'[aA]+', r'[bB]+', r'[cC]+', r'[dD]+', r'[eE]+']
                # largest section for the current contour
                largest_section_found = -1

                # print((pg_num, y_coord, pseudo_text))
                # print("----")
                for j in range(len(possible_sections)):
                    matches = re.search(r'[\[\(\|\{]+' + possible_sections[j] +r'[\]\)\}\|]+', pseudo_text, re.I)
                    if matches:
                        if j > largest_section_found:
                            largest_section_found = j

                if largest_section_found < last_section_found_number and largest_section_found != -1:
                    qn_coord.append((last_pg_num, last_y_coord, qn_num, last_section_found_number))
                    qn_num = qn_num + 1
                    ...
                    last_pg_num = pg_num
                    continue
                elif largest_section_found > last_section_found_number and largest_section_found != -1:
                    last_section_found_number = largest_section_found
                    last_y_coord = y_coord
                    last_pg_num = pg_num

                # For MCQ questions
                # Check if pseudo_text contains 4 contained in any brackets
                matches = re.search(r'[\[\(\|\{]+4[\]\)\}\|]+', pseudo_text, re.I)
                if matches:
                    qn_coord.append((pg_num, y_coord, qn_num, ""))
                    qn_num = qn_num + 1
    
    return qn_coord
    

paper_name = "2018-rosyth-p6-science"
