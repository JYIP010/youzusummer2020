...
# V10:Current implementation-working on english MCQs
# 1: Updated segmentation-images will be cropped before undergoing draw_cntr
# 2: New functions; save_qn_image and crop_image
# 3: find_qn function parameters tweaked to improve small cnt extraction
# 4: MCQ options accuracy improved
# 5: section checker capability built into find_qn_coord, updates 'Comments' section in csv
### For English MCQs Questions only ###
...
class Process:
            ...
            if w / width > 0.05 and y / height < 0.95:  # and (x/width < 0.4 or x/width > 0.5)
                ...
                if self.is_gibberish(text) or 0.35 < (w * h) / (width * height) < 0.97 or linesp is not None:
                    if h / height > 0.1 and w / h < 5:
                        ...
                        print('*******',text,linesp,(w * h) / (width * height),'w/h',w/h)
                        new_image = img[y:y + h, x:x + w]
    ...
    def generate_document(self, filename, qn_coord):
        print("STAGE 3 (Output Generation): QN " + str(self.qn_num) + "/" + str(self.total_qns))
        ...
        cv2.imwrite("TempContours/" + str(self.pg_num) + ".jpg", result)
    ...
    def save_qn_images(self, qn_coord):
        ...
        for qn_num in range(1, len(qn_coord)):
            print("STAGE 2 (Qn Images Generation): QN " + str(qn_num) + "/" + str(len(qn_coord)))
            qn = qn_coord[qn_num]
            # For last qn
    ...
    def find_qn_coords(self, filenames_list):
        ...
        for filename in filenames_list:
            print("STAGE 1 (Preprocessing): PG " + str(self.pg_number - 1) + "/" + str(self.total_pages))
            image_name = filename.replace(".jpg", "")
            ...
            for c, self.pg_number, y, w, h, xw in sorted_cntr_tuples:
                ...
                cv2.imwrite("TempImages/small_" + str(self.diagram_count) + ".jpg", new_image)
                # Read as binary image - only if we want to test what small image was read as (DEBUGGING)
                small_image = cv2.imread("TempImages/small_" + str(self.diagram_count) + ".jpg", 0)
            ...
            cv2.imwrite("TempContours/" + str(self.pg_number) + ".jpg", result)
    ...
    def main(self, pdfname):
        ...
        if img_acc > 100:
            ...
            print("Accuracy of Images : " + str(img_acc) + "%" + "\n")'''
        self.global_df.to_csv("output.csv")
